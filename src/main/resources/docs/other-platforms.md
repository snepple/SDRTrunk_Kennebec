# Stream to Icecast, Shoutcast, and IAmResponding

Beyond Broadcastify and Zello, SDRTrunk Kennebec supports several additional broadcast destinations. **Icecast** and **Shoutcast** provide live continuous MP3 audio streams compatible with any standard internet radio player. **IAmResponding** integrates with the Two Tone Detect feature for volunteer fire and EMS dispatch alerting over a local UDP connection. For routing audio to desktop applications not covered by any of the above, a virtual audio cable can bridge SDRTrunk Kennebec to any software running on the same computer.

---

## Icecast

SDRTrunk Kennebec supports two Icecast connection modes that match different server versions. Both stream MP3 audio from a mono, 8000 Hz source.

### Icecast 2 (v2.4 and later)
...
| **Mount Point** | Stream path starting with `/`, for example `/radio.mp3` |
| **Password** | The source password from your Icecast server's `icecast.xml` |
...
**Icecast TCP** uses the older TCP source handshake protocol compatible with Icecast 2.3.x servers, including the Icecast 2.3.2 variant used internally by Broadcastify. Choose this type when your self-hosted server runs an older Icecast build.
...
Shoutcast v1 uses simple password-only source authentication. It does not require a username or stream ID.
...
**1. Confirm IAmResponding is running**
