 ...
 */
public abstract class SwingTunerEditor<T extends Tuner,C extends TunerConfiguration> extends JPanel
        implements IDiscoveredTunerStatusListener, Listener<TunerEvent>, ITunerEditor
{
    private Logger mLog = LoggerFactory.getLogger(SwingTunerEditor.class);
     ...
     */
    public SwingTunerEditor(UserPreferences userPreferences, TunerManager tunerManager, DiscoveredTuner discoveredTuner)
    {
    ...
    protected JButton getInfoConfigButton()
    {
        if(mInfoConfigButton == null)
        {
            ...
            mInfoConfigButton.addActionListener(e -> {
                ...
                String friendlyName = getFriendlyNameTextField().getText();

                Platform.runLater(() -> {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Tuner Info/Config");
                    alert.setHeaderText("Tuner Configuration");
                    ...
                    if (info != null && !info.isEmpty()) {
                        grid.add(new Label(info), 0, row, 2, 1);
                        row++;
                    }

                    grid.add(new Label("Friendly Name:"), 0, row);
                    TextField fxFriendlyName = new TextField(friendlyName);
                    grid.add(fxFriendlyName, 1, row);

                    alert.getDialogPane().setContent(grid);

                    alert.showAndWait();

                    // Update the swing component if it changed
                    if (!fxFriendlyName.getText().equals(friendlyName)) {
                        javax.swing.SwingUtilities.invokeLater(() -> {
                            getFriendlyNameTextField().setText(fxFriendlyName.getText());
                        });
                    }
                });
            });
    ...
    protected FrequencyTextField getMinimumFrequencyTextField()
    {
        if(mMinimumFrequencyTextField == null)
        {
            ...
            alert.setContentText(String.valueOf("Frequency value [" +
    ...
    protected void adjustForSampleRate(int sampleRate)
    {
        ...
        if(maximum - minimum < sampleRate)
        {
            ...
            alert.setContentText(String.valueOf("Unable to adjust tuner's " +
