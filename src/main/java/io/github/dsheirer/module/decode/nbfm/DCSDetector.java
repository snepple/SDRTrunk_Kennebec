...
public class DCSDetector
{
    ...
    private void handleDetection(DCSCode code)
    {
        ...
        // Real DCS mixed with voice audio produces low band ratios (0.05-0.40). A broadband
        // interference gate on that ratio rejects every valid detection. The 23-bit codeword
        // plus parity check in DCSDecoder is sufficient to reject false positives.

        // Reset loss tracking — we just got a detection
        mLossCounter = 0;
        mSamplesSinceLastDetection = 0;

        LOGGER.trace("DCS code {} detected (confirm={}/{} target={})",
                code, mConfirmationCounter, CONFIRMATION_COUNT,
