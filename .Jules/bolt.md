...
## 2024-05-18 - Avoid String.matches() in TextFormatter
**Learning:** Using `String.matches(regex)` inside JavaFX `TextFormatter` filters causes a new `Pattern` to be compiled on every single keystroke within the UI thread, creating unnecessary object allocations and performance overhead.
**Action:** Always pre-compile regex patterns as `static final Pattern` constants and use `PATTERN.matcher(string).matches()` in high-frequency paths like UI input filters.
