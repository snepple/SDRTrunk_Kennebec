...
## 2026-05-22 - [RadioIdEditor Smart Dropdown] **Learning:** [When converting manual inputs to ComboBoxes for alias identifiers (like Radio IDs), extracting existing mapped values from PlaylistManager.getAliasModel().getAliases() significantly reduces user cognitive load by pre-populating known IDs.] **Action:** [When modifying alias identifier editors, pass the PlaylistManager into the editor to enable scanning of existing known IDs, wrap the IDs in an IdentifierValue object for display, and handle potential manual text inputs with an editable ComboBox and a custom StringConverter.]
