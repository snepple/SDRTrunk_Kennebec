...
**Action:** Convert manual text inputs for radio identifiers into an editable `ComboBox`. Retain an editable `StringConverter` fallback to allow users to still input new unaliased IDs while getting suggestions for existing ones.
