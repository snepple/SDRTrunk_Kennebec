...
## 2026-05-27 - Adding Tooltips to Call Management Toggles
**Learning:** Toggle switches and ComboBoxes in configuration panels (like `CallManagementPreferenceEditor.java`) that lack textual context for their underlying behaviors cause user friction and fail accessibility checks.
**Action:** Always verify if interactive controls in JavaFX preference editors lack tooltips and add descriptive `Tooltip` objects explaining their specific functionality (e.g., "Detect duplicate calls by matching radio identifiers.") to improve context and accessibility.
