...
## 2026-05-20 - MapPanel Action Button Accessibility and Feedback
**Learning:** The MapPanel track management action buttons (Clear, Delete, Replot) lack accessibility context and functional explanations, violating Apple HIG's Feedback and Consistency pillars. Destructive actions like deletion specifically require explicit warning tooltips.
**Action:** When creating or modifying Swing `JButton`s, always assign mnemonics via `setMnemonic()`, contextual help via `setToolTipText()`, and full accessible descriptions via `getAccessibleContext().setAccessibleName/Description()`.
