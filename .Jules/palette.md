...
## 2026-05-21 - Adding Tooltips to Directory Preference Buttons
**Learning:** Action buttons in configuration panels like `DirectoryPreferenceEditor.java` (e.g., "Change..." and "Reset" buttons for folders) lack textual context or explanation for their underlying complex behaviors, causing user friction and failing accessibility checks.
**Action:** Always verify if action buttons in JavaFX preference editors lack tooltips and add descriptive `Tooltip` objects explaining their specific functionality (e.g., "Select a custom folder for the application root directory.") to improve context and accessibility.
