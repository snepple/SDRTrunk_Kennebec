...
## 2024-05-28 - [PlaybackPreferenceEditor Layout Migration]
**Learning:** Consistent layout components like `SettingsCard` and `SettingsRow` should be preferred over manual grid layouts for preference tabs, adhering to Apple HIG's Consistency pillar. This required changing the root layout from `HBox` to `VBox`.
**Action:** Continue replacing legacy Swing-style GridPanes in preferences with SettingsCard/SettingsRow.
