...
## 2024-05-25 - [Inline Tooltips]
**Learning:** Standalone help text labels clutter the UI and violate the HIG Consistency pillar. Replacing them with inline tooltips using `createHelpIcon` and ensuring correct alignment via `setContentDisplay(ContentDisplay.RIGHT)` improves Clarity and Aesthetic Integrity.
**Action:** Always replace standalone help text labels with inline tooltips using `createHelpIcon` and ensure the help graphic aligns correctly to the right of the text.
