...
## 2024-05-23 - HIG Modernization of JMBE Library Preferences
**Learning:** Legacy UI preferences often use `GridPane` and manual padding which doesn't align with the Apple Human Interface Guidelines (HIG). Upgrading to custom HIG-compliant `SettingsCard` and `SettingsRow` provides a unified, consistent card-based look. When converting legacy forms, checkboxes that were placed naked in the layout should be grouped into rows, and action buttons can be wrapped in a row without a leading label to maintain alignment.
**Action:** Migrated JMBE Library Preference Editor to use standard `SettingsCard` and `SettingsRow` containers inside a `VBox` root instead of a `GridPane` and manual layout.
